...
interface ParsedScreeningSessionOutboxInput {
  readonly id: string
  readonly aggregateId: string
  readonly operation: ScreeningSessionOutboxOperation
  readonly payloadSchemaVersion: 'screening-session.lifecycle.v1'
...
const operations = new Set<ScreeningSessionOutboxOperation>([
  'SCREENING_SESSION_CREATED',
  'SCREENING_SESSION_CLOSED',
  'SCREENING_SESSION_REOPENED'
])
const schemaVersion = 'screening-session.lifecycle.v1'
const maximumPayloadDepth = 5
const maximumPayloadNodes = 120
const maximumPayloadObjectProperties = 50
const maximumPayloadArrayElements = 100
const maximumPayloadStringCodePoints = 500
const maximumPayloadStringUtf8Bytes = 2_048
const maximumPayloadJsonUtf8Bytes = 8_192
...
INSERT INTO sync_outbox (
  ...
  last_error_message,
  sent_at
) VALUES (?, 'SCREENING_SESSION', ?, ?, ?, ?, ?, 'PENDING', 0, NULL, NULL, NULL, NULL);
`

export function createScreeningSessionOutboxRepository(
  ...
  return Object.freeze({
    insert(
      ...
      input: InsertScreeningSessionOutboxInput
    ): void {
      assertActiveDatabaseTransactionConnection(scopedConnection)
      const parsed = parseInsertScreeningSessionOutboxInput(input)
...
function parseInsertScreeningSessionOutboxInput(
  input: InsertScreeningSessionOutboxInput
): ParsedScreeningSessionOutboxInput {
  try {
    const data = readDataProperties(input, inputKeys)
    const operation = parseScreeningSessionOutboxOperation(data.operation)
    const payloadSchemaVersion = parseScreeningSessionOutboxSchemaVersion(data.payloadSchemaVersion)
...
function parseScreeningSessionOutboxOperation(value: unknown): ScreeningSessionOutboxOperation {
  if (typeof value !== 'string' || !operations.has(value as ScreeningSessionOutboxOperation)) {
    throw new RepositoryValidationError()
  }

  return value as ScreeningSessionOutboxOperation
}

function parseScreeningSessionOutboxSchemaVersion(
  value: unknown
): 'screening-session.lifecycle.v1' {
...
function validatePayloadObject(
  ...
  try {
    ...
    const canonical: Record<string, ScreeningSessionOutboxPayloadValue> = Object.create(null)
    ...
    return deepFreeze(canonical) as ScreeningSessionOutboxPayload
  } finally {
...
function validatePayloadArray(
  ...
  try {
    ...
    const canonical: ScreeningSessionOutboxPayloadValue[] = []
    ...
    return deepFreeze(canonical) as readonly ScreeningSessionOutboxPayloadValue[]
  } finally {
    state.activeObjects.delete(value)
  }
}
...
function deepFreeze<T extends ScreeningSessionOutboxPayloadValue>(value: T): T {
