...
describe('migration runner integration', () => {
  it('upgrades a fresh HSD-006 database to schema version 21 and is idempotent', async () => {
    await withDatabase((connection) => {
      ...
      expect(readLedgerRows(connection)).toHaveLength(21)
  ...
  it('upgrades an existing schema version 3 database to schema version 21', async () => {
    await withDatabase((connection) => {
  ...
  it('refuses newer and inconsistent migration metadata without repair', async () => {
    ...
    await withDatabase((connection) => {
      ...
      connection
        ...
        .run(22, 'extra', 'a'.repeat(64), fixedClock.now(), '1.0.0')
