...
describe('migration runner integration', () => {
  it('upgrades a fresh HSD-006 database to schema version 13 and is idempotent', async () => {
    await withDatabase((connection) => {
      ...
      expect(readLedgerRows(connection)).toHaveLength(13)
  ...
  it('upgrades an existing schema version 3 database to schema version 13', async () => {
    await withDatabase((connection) => {
  ...
  it('backfills activity response semantics without rewriting existing activity rows', async () => {
    await withDatabase((connection) => {
      ...
      expect(migrate(connection).appliedVersions).toEqual([10, 11, 12, 13])
      expect(
        connection
          .prepare(
  ...
  it('refuses newer and inconsistent migration metadata without repair', async () => {
    ...
    await withDatabase((connection) => {
      ...
      connection
        ...
        .run(14, 'extra', 'a'.repeat(64), fixedClock.now(), '1.0.0')
