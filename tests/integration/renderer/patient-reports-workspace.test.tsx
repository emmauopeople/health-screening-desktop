...
describe('PatientReportsWorkspace', () => {
  ...
  it('creates a complete general browser report and opens a dedicated printable preview', async () => {
    ...
    expect(browserReport.querySelector('[data-report-chart]')).toBeNull()
    ...
    expect(browserReport.textContent).not.toContain('Lifestyle area')
    expect(browserReport.textContent).not.toContain('Complete referral details')
    expect(browserReport.textContent).toContain('Referral overview for the selected date range')
  ...
  it('creates focused Vitals, Lifestyle, and Referrals reports with exact record links', async () => {
    ...
    expect(printPreview?.querySelector('[data-report-chart]')).toBeNull()
