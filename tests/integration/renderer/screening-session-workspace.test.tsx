...
describe('screening patient entry workspace', () => {
  ...
  it('navigates from Food to OTC without locking the draft and preserves OTC rows on return', async () => {
    ...
    expect(text(mounted)).toContain('3 total drinks')
    expect(text(mounted)).toContain('Cigarette: 2 days')
    expect(text(mounted)).toContain('Exercise • Light • 2 days • 30 minutes per active day')
    expect(text(mounted)).toContain('Sedentary time: 30 minutes per day')
    expect(text(mounted)).toContain('Weekly response: Usual')
    expect(text(mounted)).toContain('Occupation: Farmer')
    expect(text(mounted)).toContain('Sport • Walking • Light • 2 days • 30 minutes per day')
    expect(text(mounted)).not.toContain('Protocol reference:')
    expect(text(mounted)).not.toContain(
      'Protocol action, referral decision, and final completion are not performed in this step.'
    )
    expect(buttonByText(mounted, 'Edit Vitals').disabled).toBe(false)
    expect(buttonByText(mounted, 'Edit Weekly lifestyle').disabled).toBe(false)
    expect(buttonByText(mounted, 'Edit Food').disabled).toBe(false)
    expect(buttonByText(mounted, 'Edit OTC medications').disabled).toBe(false)
    expect(buttonByText(mounted, 'Complete screening').disabled).toBe(true)

    await clickButton(mounted, 'Edit OTC medications')
    expect(mounted.container.querySelector('#screening-otc-step-title')).not.toBeNull()
    expect(inputByLabel(mounted, 'Medication name').value).toBe('Ibuprofen')
    await clickButton(mounted, 'Continue')
    expect(mounted.container.querySelector('#screening-review-step-title')).not.toBeNull()
